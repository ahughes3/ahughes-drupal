...
/**
 * Makes sure the path has only path separators native for the current OS.
 */
...
/**
 * Cross-platform compatible helper to recursively create a directory tree.
 *
 * @param string $path
 ...
 *   drush_mkdir.
 *   If $required is FALSE, then a different location should be selected, and
 *   a final.
...
function drush_mkdir($path, $required = TRUE) {
  if (!is_dir($path)) {
    if (drush_mkdir(dirname($path))) {
      ...
      elseif (is_dir($path) && is_writable($path)) {
        ...
        if (is_writable(dirname($path))) {
          return dt('Unable to create !dir.', [
            '!dir' => preg_replace('/\w+\/\.\.\//', '', $path),
          ]);
        }
        else {
          return dt(
            'Unable to create !newdir in !dir. Please check directory permissions.',
            [
    ...
    if (!is_writable($path)) {
      ...
      return dt(
        'Directory !dir exists, but is not writable. Please check directory permissions.',
        [
          '!dir' => realpath($path),
        ]
      );
